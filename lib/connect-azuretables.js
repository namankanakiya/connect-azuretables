...
module.exports = function(session) {
    ...
    AzureTablesStore.prototype.cleanUp = function(sessionTimeOut) {
        var deleteAfter = Date.now() - sessionTimeOut * 60000;
        var query = new azure.TableQuery().where('lastTouched gt ?', deleteAfter);
        var store = this;
        ...
        function deleteEntry(entry) {
            store.tableService.deleteEntity(store.table, entry, function(error, result) {
                if (error) {
                    ...
                    store.log('cleaned up session ' + entry.PartitionKey);
                }
            });
        }
