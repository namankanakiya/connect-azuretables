...
module.exports = function(session) {
    ...
    function AzureTablesStore(options) {
        ...
        */
        ...
        if (options.sessionTimeOut) {
            ...
            self.log('starting session cleanup job with session timeout ' + options.sessionTimeOut + ' minutes, cron ' + cronPattern);
            new CronJob(cronPattern, runCleanUp, null, true);
        }
        
        function runCleanUp() {
            self.log('running cron job ' + (new Date()).toUTCString());
    ...
    AzureTablesStore.prototype.update = function(method, sid, data, fn) {
        ...
        var session = {
            ...
            lastTouched: entGen.Int64(Date.now())
        };
    ...
    AzureTablesStore.prototype.cleanUp = function(sessionTimeOut) {
        console.log('cleaning up with timeout ' + sessionTimeOut);
    };
