...
module.exports = function(session) {
    ...
    AzureTablesStore.prototype.set = function(sid, data, fn) {
        this.log('connect-azuretables called SET ' + sid);
        this.update(sid, data, fn);
    };

    //touch - optional function
    AzureTablesStore.prototype.touch = function(sid, data, fn) {
        this.log('connect-azuretables called TOUCH ' + sid);
        this.update(sid, data, fn);
    };
    
    AzureTablesStore.prototype.update = function(sid, data, fn) {
        var store = this;
        var cleanSid = sanitize(sid);
        var entGen = azure.TableUtilities.entityGenerator;
        var session = {
