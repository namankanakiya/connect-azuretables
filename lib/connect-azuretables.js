...
var debug = require('debug')('connect:azuretables');

var DEFAULT_TABLE = 'connect-azuretables-sessions';
var RETRY_LIMIT = 3;
...
module.exports = function(session) {
    ...
    function AzureTablesStore(options) {
        ...
        if these are not found, AZURE_STORAGE_ACCOUNT and AZURE_STORAGE_ACCESS_KEY
        must be supplied on options
        */
        
        //todo: allow retry policy to bet set on options
        var retryOperations = new azure.LinerarRetryPolicyFilter(RETRY_LIMIT, RETRY_INTERVAL);
    ...
    AzureTablesStore.prototype.get = function(sid, fn) {
        ...
        store.tableService.retrieveEntity(store.table, cleanSid, cleanSid, function(error, result) {     
            return error ? fn(error) : fn(null, JSON.parse(result.data)); 
        });  
    }
    ...
    AzureTablesStore.prototype.set = function(sid, data, fn) {
        ...
        store.tableService.insertOrReplace(store.table, session, function(error, result) {     
            return error ? fn(error) : fn(null, true); 
        });  
    }
